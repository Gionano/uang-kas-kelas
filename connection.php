...
function addBulanPembayaran($data) {
	...
	$libur_minggu_1 = isset($data['libur_minggu_1']) ? 1 : 0;
	$libur_minggu_2 = isset($data['libur_minggu_2']) ? 1 : 0;
	$libur_minggu_3 = isset($data['libur_minggu_3']) ? 1 : 0;
	$libur_minggu_4 = isset($data['libur_minggu_4']) ? 1 : 0;
	...
	if (mysqli_fetch_assoc($check_bulan)) {
		...
		$query = mysqli_query($conn, "INSERT INTO bulan_pembayaran (nama_bulan, tahun, pembayaran_perminggu, libur_minggu_1, libur_minggu_2, libur_minggu_3, libur_minggu_4) VALUES ('$nama_bulan', '$tahun', '$pembayaran_perminggu', '$libur_minggu_1', '$libur_minggu_2', '$libur_minggu_3', '$libur_minggu_4')");
...
function editBulanPembayaran($data) {
	...
	$libur_minggu_3 = isset($data['libur_minggu_3']) ? 1 : 0;
	$libur_minggu_4 = isset($data['libur_minggu_4']) ? 1 : 0;
	$query = mysqli_query($conn, "UPDATE bulan_pembayaran SET nama_bulan = '$nama_bulan', tahun = '$tahun', pembayaran_perminggu = '$pembayaran_perminggu', libur_minggu_1 = '$libur_minggu_1', libur_minggu_2 = '$libur_minggu_2', libur_minggu_3 = '$libur_minggu_3', libur_minggu_4 = '$libur_minggu_4' WHERE id_bulan_pembayaran = '$id_bulan_pembayaran'");
  	return mysqli_affected_rows($conn);
}

function updateStatusLunas($id_uang_kas) {
    global $conn;
    $query_uk = mysqli_query($conn, "SELECT * FROM uang_kas INNER JOIN bulan_pembayaran ON uang_kas.id_bulan_pembayaran = bulan_pembayaran.id_bulan_pembayaran WHERE id_uang_kas = '$id_uang_kas'");
    $data = mysqli_fetch_assoc($query_uk);

    $pembayaran_perminggu = $data['pembayaran_perminggu'];
    $total_minggu_bayar = 4;
    if ($data['libur_minggu_1']) $total_minggu_bayar--;
    if ($data['libur_minggu_2']) $total_minggu_bayar--;
    if ($data['libur_minggu_3']) $total_minggu_bayar--;
    if ($data['libur_minggu_4']) $total_minggu_bayar--;
    
    $total_harus_bayar = $pembayaran_perminggu * $total_minggu_bayar;

    $total_sudah_bayar = $data['minggu_ke_1'] + $data['minggu_ke_2'] + $data['minggu_ke_3'] + $data['minggu_ke_4'];

    if ($total_sudah_bayar >= $total_harus_bayar && $total_harus_bayar > 0) {
        mysqli_query($conn, "UPDATE uang_kas SET status_lunas = '1' WHERE id_uang_kas = '$id_uang_kas'");
    } else {
        mysqli_query($conn, "UPDATE uang_kas SET status_lunas = '0' WHERE id_uang_kas = '$id_uang_kas'");
    }
}
...
function editPembayaranUangKas($data) {
	...
	if (isset($_POST['minggu_ke_1'])) {
		$field_to_update = 'minggu_ke_1';
		$minggu_ke = 'minggu ke-1';
		$uang_baru = htmlspecialchars($_POST['minggu_ke_1']);
		$uang_baru_numeric = ($uang_baru == '') ? 0 : $uang_baru;
	} elseif (isset($_POST['minggu_ke_2'])) {
		$field_to_update = 'minggu_ke_2';
		$minggu_ke = 'minggu ke-2';
		$uang_baru = htmlspecialchars($_POST['minggu_ke_2']);
		$uang_baru_numeric = ($uang_baru == '') ? 0 : $uang_baru;
	} elseif (isset($_POST['minggu_ke_3'])) {
		$field_to_update = 'minggu_ke_3';
		$minggu_ke = 'minggu ke-3';
		$uang_baru = htmlspecialchars($_POST['minggu_ke_3']);
		$uang_baru_numeric = ($uang_baru == '') ? 0 : $uang_baru;
	} elseif (isset($_POST['minggu_ke_4'])) {
		$field_to_update = 'minggu_ke_4';
		$minggu_ke = 'minggu ke-4';
		$uang_baru = htmlspecialchars($_POST['minggu_ke_4']);
		$uang_baru_numeric = ($uang_baru == '') ? 0 : $uang_baru;
	}

	if ($field_to_update != '') {
		$uang_sebelum = htmlspecialchars($data['uang_sebelum']);
		$uang_sebelum_numeric = ($uang_sebelum == '') ? 0 : $uang_sebelum;

		$query = mysqli_query($conn, "UPDATE uang_kas SET $field_to_update = '$uang_baru_numeric' WHERE id_uang_kas = '$id_uang_kas'");
		$affected_rows = mysqli_affected_rows($conn);

		if ($affected_rows > 0) {
			riwayat($id_user, $id_uang_kas, "telah mengubah pembayaran $minggu_ke dari Rp. " . number_format($uang_sebelum_numeric) . " menjadi Rp. " . number_format($uang_baru_numeric));
			updateStatusLunas($id_uang_kas);
		}
...
function bayarDenda($data) {
    ...
    $uang_kas_data = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM uang_kas INNER JOIN bulan_pembayaran ON uang_kas.id_bulan_pembayaran = bulan_pembayaran.id_bulan_pembayaran WHERE id_uang_kas = '$id_uang_kas'"));
    $denda = $uang_kas_data['denda_per_bulan'];

    $query = mysqli_query($conn, "UPDATE uang_kas SET denda = '$denda', status_denda = '1' WHERE id_uang_kas = '$id_uang_kas'");
    
    if (mysqli_affected_rows($conn) > 0) {
        riwayat($id_user, $id_uang_kas, "telah membayar denda sebesar Rp. " . number_format($denda));
    }

    return mysqli_affected_rows($conn);
}

function hapusDenda($id_uang_kas) {
    global $conn;
    if (checkJabatan() == true) {
        $id_user = $_SESSION['id_user'];
        $uang_kas_data = mysqli_fetch_assoc(mysqli_query($conn, "SELECT * FROM uang_kas WHERE id_uang_kas = '$id_uang_kas'"));
        $denda_sebelumnya = $uang_kas_data['denda'];
        $query = mysqli_query($conn, "UPDATE uang_kas SET denda = '0', status_denda = '0' WHERE id_uang_kas = '$id_uang_kas'");
        if (mysqli_affected_rows($conn) > 0) {
            riwayat($id_user, $id_uang_kas, "telah membatalkan pembayaran denda sebesar Rp. " . number_format($denda_sebelumnya));
        }
        return mysqli_affected_rows($conn);
    }
    return 0;
